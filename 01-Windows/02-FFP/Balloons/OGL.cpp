...
{
	//Function Prototypes
	void Balloon(float, float, float, float, float, float, float, float);
	...
	Balloon(-1.0f, 1.5f, -1.0f, 0.3f, 0.0f, 1.0f, 0.0f, 0.2f);
	Balloon(-0.8f, 1.5f, -1.0f, 0.2f, 1.0f, 0.0f, 0.0f, 0.2f);
	Balloon(-0.7f, 1.5f, -1.0f, 0.2f, 0.0f, 0.0f, 1.0f, 0.2f);
	Balloon(1.0f, 1.5f, -1.0f, 0.3f, 1.0f, 1.0f, 0.0f, 0.2f);
	Balloon(0.8f, 1.5f, -1.0f, 0.3f, 0.0f, 1.0f, 1.0f, 0.2f);
	Balloon(0.7f, 1.5f, -1.0f, 0.2f, 1.0f, 0.0f, 1.0f, 0.2f);

	SwapBuffers(ghdc);
}

void Balloon(float x, float y, float z, float size, float col_red, float col_green, float col_blue, float scale)
{
	//variable declarations
	float angle, a, k;
	static float b = -0.2f;
	...
	glScalef(1.0f, 1.0f + scale, 1.0f);
	glTranslatef(x, y + (0.2f * sinf(b)), z);
	...
	for (float j = -300.0f; j < 300.0f; j++)
	{
		angle = (2.0f * PI) / 360.0f;
		a = 0.1f;
		k = 0.001f;

		x = a * exp(k * angle * j) * cosf(angle * j);
		y = a * exp(k * angle * j) * sinf(angle * j);

		glColor3f(1.0f, 0.0f, 0.0f);
		glVertex3f(x, y, ((j + 300.0f) / 800.0f));
	}
