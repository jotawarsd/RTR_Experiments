...
void display(void)
{
	...
	for (theta = 0.0f; theta < 180.0f; theta += 4.0f)
	{
		glBegin(GL_TRIANGLE_STRIP);
		for (phi = -180.0; phi <= 180.0f; phi += 4.0f)
		{
			glTexCoord2f(
				r * sinf(phi * M_PI / 180.0f) / 360.0f,
				r * cosf(theta * M_PI / 180.0f) / 180.0f
			);
			glVertex3f(
				r * sinf(theta * M_PI / 180.0f) * cosf(phi * M_PI / 180.0f),
				r * sinf(theta * M_PI / 180.0f) * sinf(phi * M_PI / 180.0f),
				r * cosf(theta * M_PI / 180.0f)
			);

			glTexCoord2f(
				r * sinf(phi * M_PI / 180.0f) / 360.0f,
				r * cosf((theta + 4.0f) * M_PI / 180.0f) / 180.0f
			);
			glVertex3f(
				r * sinf((theta + 4.0f) * M_PI / 180.0f) * cosf(phi * M_PI / 180.0f),
				r * sinf((theta + 4.0f) * M_PI / 180.0f) * sinf(phi * M_PI / 180.0f),
				r * cosf((theta + 4.0f) * M_PI / 180.0f)
			);
		}
		glEnd();
	}
