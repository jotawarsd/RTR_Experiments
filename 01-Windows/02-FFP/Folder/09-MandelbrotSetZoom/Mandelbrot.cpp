...
void display(void)
{
	...
	glTranslatef(0.7520f, -0.0355f, -0.00026f);

	for (i = -0.37605f; i < -0.37595f; i += 0.00000006)
	{
		p.x = 2.0f * i;
		for (j = 0.0177f; j < 0.0178f; j += 0.00000011)
		{
			p.y = 2.0f * j;
			x = 0;
			y = 0;
			...
			glBegin(GL_POINTS);
			if (k >= 2 && k != ITER)
				glColor3f(k / (0.54f * ITER), k / (0.7f * ITER), k / (0.27f * ITER));
			else if (k == ITER)
