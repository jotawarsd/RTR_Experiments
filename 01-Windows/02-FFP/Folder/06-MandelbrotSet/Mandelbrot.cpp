...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		...
		TEXT("Shaunak Jotawar Mandelbrot Set"),													//Title bar text
		WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE,					//style of window
		0.0f,																					//x coordinate of top left corner
		0.0f,																					//y coordinate of top left corner
		WIN_WIDTH,																				//width
		WIN_HEIGHT,																				//height
...
int initialize(void)
{
	...
	glClearColor(2 / (0.054f * ITER), 2 / (0.07f * ITER), 2 / (0.027f * ITER), 1.0f);
...
void display(void)
{
	...
	for (i = -2.0f; i < 1.0f; i += 0.0008)
	{
		p.x = 2.0f * i;
		for (j = -1.23f; j < 1.23f; j += 0.0008)
		{
			p.y = 2.0f * j;
			x = 0;
			y = 0;
