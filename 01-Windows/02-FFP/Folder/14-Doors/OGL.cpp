...
int initialize(void)
{
	...
	LoadGLTexture(&textureStone, MAKEINTRESOURCE(IDBITMAP_STONE));
	LoadGLTexture(&textureRoof, MAKEINTRESOURCE(IDBITMAP_ROOF));
...
void hut(float length, float breadth, float height)
{
	glEnable(GL_TEXTURE_2D);
	...
	glVertex3f(-length, height / 2.5f, breadth);
	...
	glVertex3f(length, height / 2.5f, breadth);
	glTexCoord2f(0.0f, 0.5f);
	glVertex3f(length / 2.0f, height / 2.5f, breadth);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3f(length / 2.0f, -height / 2.5f, breadth);
	...
	//Top
	glTexCoord2f(2.0f, 0.0f);
	glVertex3f((length + (length / 4.0f)), height, breadth);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3f(-(length + (length / 4.0f)), height, breadth);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3f(-(length + (length / 4.0f)), height + (height / 1.5f), 0.0f);
