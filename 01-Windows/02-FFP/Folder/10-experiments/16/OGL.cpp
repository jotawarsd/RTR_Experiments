...
#define G 0.00000000643
#define M 9999000000000000000
#define R 1.8
#define C 520000
...
{
	//variable declarations
	float x, y, xi, yi, xai, yai;
	float a, r, ri;
	float dtheta, ditheta, daitheta;
	float b, bi;
	...
	for (r = 0.5f; r < 2.1f; r += 0.1f)
	{
		glBegin(GL_TRIANGLE_STRIP);
		for (a = angle - M_PI; a <= angle + 0.1f + M_PI; a += M_PI / 32.0f)
		{
			x = r * cosf(a);
			y = r * sinf(a);
			xi = (r + 0.1f) * cosf(a);
			yi = (r + 0.1f) * sinf(a);
			xai = r * cosf(a + (M_PI / 32.0f));
			yai = r * sinf(a + (M_PI / 32.0f));
			ri = r + 0.1;
			b = (3 * sqrt(3) * M) + ((sqrt(3.0) / 2.0) * (pow(r - (3.0 * M), 2.0) / M));
			bi = (3 * sqrt(3) * M) + ((sqrt(3.0) / 2.0) * (pow(ri - (3.0 * M), 2.0) / M));

			dtheta = y * r / sqrt((2.0 * M * r) - pow(r, 2.0) + pow(r, 6.0));
			ditheta = yi * ri / sqrt((2.0 * M * ri) - pow(ri, 2.0) + pow(ri, 6.0));

			//glTranslatef(0.0f, 0.0f, 0.0f);
			if (y < 0.0f)
			{
				glVertex3f(x, y, cos(dtheta));
				glVertex3f(xi, yi, sin(ditheta));
			}
			else
			{
				glVertex3f(x, y, 0.0f);
				glVertex3f(xi, yi, 0.0f);
