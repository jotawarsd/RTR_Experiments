...
#define G 0.00000000006743
#define M 8700000000000000000
#define C 30000000
...
void display(void)
{
	...
	glColor3f(0.05f, 0.0f, 0.5f);

	gluSphere(quadric, 0.3f, 60, 60);
	glPopMatrix();
	...
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	for (radius = 0.4f; radius <= 1.1f; radius += 0.007f)
	{
		for (i = k; i < k + 400.0f; i++)
		{
			glBegin(GL_POINTS);
			...
			if (y < 0.0f)
			{
				dtheta = y * y * pow(radius, -0.1f) * 2.0f * G * M / ((pow(y, 2.0f) + pow(x, 2.0f)) * (C * C)) ;
				//glTranslatef(0.0f, 0.0f, 4.0f * sinf(dtheta));
				//glTranslatef(x, y, 0.0f);

				glBegin(GL_POINTS);
				glVertex3f(x, -0.5 * pow(y, 2.0f), y * pow(tanf(dtheta), 2.0f));
				glEnd();
				glBegin(GL_POINTS);
				glVertex3f(x, -0.5 * pow(y, 2.0f), -y * pow(tanf(dtheta), 2.0f));
				glEnd();
