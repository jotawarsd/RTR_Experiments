...
#define G 0.00000008743
#define M 7772190000000000000
#define R 13
#define C 600000
...
void display(void)
{
	...
	for (radius = 0.18f; radius <= 1.0f; radius += 0.009f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			...
			x = radius * cosf(angle);
			y = radius * sinf(angle);
			dtheta = y * pow(radius, -1.10) * 4.7 * G * M / (R * pow(C, 2.0));
			if (y < 0.0f && radius >= 0.3f)
				glVertex3d(x, y, -y * 1.8 * pow(sinf(dtheta), 3.0));
			else if (radius > 0.3f && radius < 0.7f)
			{
				glVertex3d(x, y, y * 2.0 * pow(sinf(dtheta), 4.0));
				glVertex3f(x, y, 0.0f);
			}
			else if (radius == 0.18f)
			{
				glVertex3f(x, 0.0f, y);
			}
			else if (radius >= 0.4f)
