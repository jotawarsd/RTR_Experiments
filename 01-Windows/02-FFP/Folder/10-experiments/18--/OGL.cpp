...
void display(void)
{
	...
	for (r = 0.5f; r < 2.1f; r += 0.1f)
	{
		...
		//glTexCoord2f(r * cosf(a), r * sinf(a));
		for (a = angle; a <= angle + 0.1f + 2.0 * M_PI; a += M_PI / 32.0f)
		{
			x = r * cosf(a);
			y = r * sinf(a);
			xi = (r + 0.1f) * cosf(a);
			...
			dtheta = 1 + sqrt(1 - (pow(0.96, 2.0) * cos(a)));
			//ditheta = 1 + sqrt(pow(M, 2.0) - (pow(0.96 / M, 2.0) * cos(a)));
			...
			{*/
				glVertex3f(r * cos(dtheta), r * sin(dtheta), 0.0f);
				glVertex3f(ri * cos(dtheta), ri * sin(dtheta), 0.0f);
			//}
		}
		glEnd();
	}
