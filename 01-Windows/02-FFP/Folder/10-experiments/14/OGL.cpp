...
{
	//variable declarations
	double x, y, xi, yi, xai, yai;
	double a, r, ri;
	double dtheta, ditheta, daitheta;
	double a_disk;
	...
	for (r = 0.5; r <= 4.0; r += 0.1)
	{
		glBegin(GL_TRIANGLE_STRIP);
		for (a = angle - (M_PI / 1.0); a <= angle + 0.01 + (M_PI / 1.0); a += M_PI / 32.0)
		{
			x = r * cos(a);
			y = r * sin(a);
			xi = (r + 0.1) * cos(a);
			yi = (r + 0.1) * sin(a);
			xai = r * cos(a + (M_PI / 32.0));
			yai = r * sin(a + (M_PI / 32.0));
			ri = r + 0.1;

			dtheta = sqrt(1 + tan(8.0) * tan(8.0) * sin(a) * sin(a));
			ditheta = sqrt(1 + tan(8.0) * tan(8.0) * sin(a) * sin(a));

			//glTranslatef(0.0f, 0.0f, 0.0f);
			/*if (xi > -0.05f)
			{
				glVertex3d(x, -dtheta, y);
				glVertex3d(xi,-ditheta, yi);
			}
			else
			{*/
				glVertex3f(x, y, -r * (dtheta));
				glVertex3f(xi, yi, -ri * (ditheta));
			//}
		}
		glEnd();
	}
