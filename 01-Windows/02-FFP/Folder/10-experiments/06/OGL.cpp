...
void display(void)
{
	...
	glPushMatrix();
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	for (radius = 0.5f; radius <= 2.0f; radius += 0.007f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			...
			dtheta = 4.0f * G * M / (radius * pow(C, 2.0));
			if (y < 0.0f)
			{
				dx = y * sinf(dtheta / 2.0f);
				dz = y * cosf(dtheta / 2.0f);
				//glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
				glBegin(GL_POINTS);
				glVertex3f(x, y, sinf(y) * cosf(x * 0.8f));
				glEnd();
			}
			else
			{
