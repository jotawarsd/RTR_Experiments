...
#define G 0.00000006743
#define M 5972190000000000000
#define R 7
#define C 520000
...
void display(void)
{
	...
	//draw sun sphere
	gluSphere(quadric, 0.45f, 60, 60);
	glPopMatrix();		//restore saved camera matrix

	glPushMatrix();
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	for (radius = 0.5f; radius <= 1.5f; radius += 0.007f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			...
			x = radius * cosf(angle);
			y = radius * sinf(angle);
			dtheta = y * pow(radius, -2.0f) * 3.0f * G * M / (R * pow(C, 2.0));
			if (y < 0.0f)
				glVertex3d(x, y, (pow(y, 2.0f) - pow(x, 2.0f)) * sinf(dtheta));
			else
				glVertex3f(x, y, 0.0f);
	...
	for (radius = 0.7f; radius <= 1.0f; radius += 0.007f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			glBegin(GL_POINTS);
			...
			x = radius * cosf(angle);
			y = radius * sinf(angle);
			dtheta = y * 3.0f * G * M / (R * pow(C, 2.0) * pow(radius, 4.0f));
			if (y < 0.0f)
				glVertex3d(x, 0.2f * y, -(pow(radius, 2) - pow(x, 2)) * sinf(dtheta));
			else
				glVertex3f(x, y, 0.0f);
