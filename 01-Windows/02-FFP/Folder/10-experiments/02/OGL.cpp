...
void display(void)
{
	...
	//create quadric
	glColor4f(0.05f, 0.0f, 0.2f, 1.0f);
	//draw sun sphere
	...
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	for (radius = 0.5f; radius <= 2.0f; radius += 0.01f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			...
			x = radius * cosf(angle);
			y = radius * sinf(angle);
			dtheta = y * 4.0f * G * M / (R * pow(C, 2.0)) / pow(radius, 0.5);
			yf = ((y - radius * cosf(dtheta)) / pow(radius, 1.2)) + 1.0;
			if (y < 0.0f)
			{
				glVertex3d(x, yf, yf * pow(tanf(dtheta), 2.0f));
				if (radius < 1.2f)
					glVertex3d(x, yf, -yf * pow(tanf(dtheta), 2.0f));
			}
			else
