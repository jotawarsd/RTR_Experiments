...
void display(void)
{
	...
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	glBegin(GL_POINTS);
	for (radius = 0.25f; radius <= 1.5f; radius += 0.007f)
	{
		for (i = k; i < k + 800.0f; i++)
		{
			...
			x = radius * cosf(angle);
			y = radius * sinf(angle);
			dtheta = y * pow(radius, -1.15) * 4.5 * G * M / (R * pow(C, 2.0));
			if (y < 0.0f)
			{
				if (radius > 0.32f)
					glVertex3f(x, y, y * 0.83f * pow(sinf(dtheta), 2.0));
				if (radius < 1.0f && radius > 0.35)
					glVertex3f(x, y, -y * pow(sinf(dtheta), 2.0));
				if (radius < 0.256f)
					glVertex3f(-x, 0.0f, y);
			}
			else
