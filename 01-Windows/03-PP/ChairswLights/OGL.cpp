...
GLfloat materialSpecular[] = { 0.7, 0.7, 0.7, 1.0f };
...
int initialize(void)
{
	...
	glVertexAttrib3f(SDJ_ATTRIBUTE_COLOR, 0.0f, 1.0f, 1.0f);
...
void display(void)
{
	...
	mat4 scaleMatrix = scale(0.75f, 0.75f, 0.75f);
	...
	for (int j = 0; j < 10; j++)
	{
		...
		{
			if (j % 2 != 0)
				translationMatrix = translate((GLfloat)i - 0.5f, -1.0f + ((GLfloat)j / 2.0f), -10.0f - (GLfloat)j);
			else
				translationMatrix = translate((GLfloat)i, -1.0f + ((GLfloat)j / 2.0f), -10.0f - (GLfloat)j);
