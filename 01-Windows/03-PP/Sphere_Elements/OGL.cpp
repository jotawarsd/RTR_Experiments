...
GLuint g_cyLinderStacks = 200;
GLuint g_cylinderSlices = 200;
GLfloat g_height = 2.0f;
GLfloat g_radius = 2.0f;
...
int initialize(void)
{
	...
	*/
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(o_texcoord, 1.0f, 1.0f) * vec4(o_normal.xyz, 1.0f);" \
		"}";
	...
	createSphere(g_cyLinderStacks, g_cylinderSlices, g_radius, g_height);
...
void createSphere(GLuint stacks, GLuint slices, GLfloat radius, GLfloat height)
{
	...
	g_vertexCount = slices * stacks;
	g_indexCount = (slices - 1) * (stacks - 1) * 3 * 2;

	GLuint *indices;

	GLfloat *position = (GLfloat *)malloc(3 * g_vertexCount * sizeof(GLfloat));
	GLfloat *normals = (GLfloat *)malloc(3 * g_vertexCount * sizeof(GLfloat));
	GLfloat *texCoords = (GLfloat *)malloc(2 * g_vertexCount * sizeof(GLfloat));

	indices = (GLuint *)malloc(g_indexCount * sizeof(GLuint));
	...
	for (i = 0; i < stacks; i++)
	{
		for (j = 0; j < slices; j++)
		{
			...
			s = (float)j / (float)(slices - 1);
			t = (float)i / (float)(stacks - 1);

			position[3 * vertexptr + 0] = x * radius;
			position[3 * vertexptr + 1] = y * radius;
			position[3 * vertexptr + 2] = z * radius;

			normals[3 * vertexptr + 0] = x;
			normals[3 * vertexptr + 1] = y;
			normals[3 * vertexptr + 2] = z;
	...
	//vertex array object
	size_t size = g_vertexCount * sizeof(GLfloat);
	...
	glBindVertexArray(vaoCylinder);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, g_indexCount * sizeof(GLuint), indices, GL_STATIC_DRAW);
...
void display(void)
{
	...
	glDrawElements(GL_POINTS, g_indexCount, GL_UNSIGNED_INT, 0);
