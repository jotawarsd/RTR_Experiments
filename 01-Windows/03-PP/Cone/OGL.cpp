...
GLfloat g_height = 1.0f;
GLfloat g_radius = 1.0f;
...
void createCylinder(GLuint stacks, GLuint slices, GLfloat radius, GLfloat height)
{
	...
	for (i = 0; i < stacks - 1; i++)
	{
		for (j = 0; j < slices - 1; j++)
		{
			...
			position[3 * vertexptr + 0] = x * radius_d;
			position[3 * vertexptr + 1] = y * height - (height / 2.0f);
			position[3 * vertexptr + 2] = z * radius_d;
