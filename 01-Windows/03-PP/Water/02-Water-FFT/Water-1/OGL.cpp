...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"for (int i = 0; i < 5; i++)"
		"{" \
		"fi = 4.0f + float(i * 16.0) * abs(sin(a_position.z));" \
		"wi = sin(a_position.x * fi + u_time) * 0.01;" \
		"w += wi;"
		"}" \
		"vec4 s_position = vec4(a_position.x, w * cos(a_position.z), a_position.z, 1.0f);" \
...
void display(void)
{
	...
	translationMatrix = translate(0.0f, -0.1f, -1.0f);
