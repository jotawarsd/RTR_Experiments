...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"{" \
		"fi = 4.0 * sqrt(i);" \
		"wxi = (a_position.x - 0.2 * sin(a_position.x * 2.0 * sqrt(i) + fi * u_time + (i * a_position.z * a_position.z * i))) * 0.15;" \
		"wyi = cos(a_position.x * 2.0f * sqrt(i) + fi * u_time) * 0.01;" \
		"wx += wxi;"
		"wy += wyi;"
		"}" \
		"vec4 s_position = vec4(wx, wy, a_position.z, 1.0f);" \
...
void display(void)
{
	...
	translationMatrix = translate(0.0f, -0.05f, -1.2f);
