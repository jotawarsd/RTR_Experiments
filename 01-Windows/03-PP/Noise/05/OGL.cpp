...
float volumetric_cube_data[CUBE_SIZE][CUBE_SIZE][CUBE_SIZE][3];
...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode =
		...
		/*"float intensity = abs(noiseVec[0]) + abs(noiseVec[1]) + abs(noiseVec[2]) + abs(noiseVec[3]);" \
		"intensity *= 1.5;" \*/
	...
	for (int z = 0; z < CUBE_SIZE; z++)
	{
		for (int y = 0; y < CUBE_SIZE; y++)
		{
			...
			{
				volumetric_cube_data[x][y][z][0] = 3.0f * (x - CUBE_SIZE / 2.0f) / (CUBE_SIZE / 2.0f);
				volumetric_cube_data[x][y][z][1] = 3.0f * (y - CUBE_SIZE / 2.0f) / (CUBE_SIZE / 2.0f);
				volumetric_cube_data[x][y][z][2] = 3.0f * (z - CUBE_SIZE / 2.0f) / (CUBE_SIZE / 2.0f);
			}
		}
	}
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(volumetric_cube_data), volumetric_cube_data, GL_STATIC_DRAW);
...
void display(void)
{
	...
	rotationMatrix = rotate(rotationX, 1.0f, 0.0f, 0.0f) * rotate(rotationY, 0.0f, 1.0f, 0.0f) * rotate(rotationZ, 0.0f, 0.0f, 1.0f);
