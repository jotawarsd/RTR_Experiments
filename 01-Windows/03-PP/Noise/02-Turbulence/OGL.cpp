...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"mc_Position = u_modelMatrix * a_position;" \
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"vec4 noiseVec = texture(u_textureSampler, vec3(mc_Position.x * noiseScale, mc_Position.y * noiseScale, u_offset));" \
		"float intensity = abs(noiseVec[0] - 0.25) + abs(noiseVec[1] - 0.125) + abs(noiseVec[2] - 0.0625) + abs(noiseVec[3] - 0.03125);" \
		"intensity = clamp(intensity * 1.0, 0.0, 1.0) * 4.5;" \
		"FragColor = vec4(mix(Color1, Color2, intensity), 1.0);" \
		"}";
