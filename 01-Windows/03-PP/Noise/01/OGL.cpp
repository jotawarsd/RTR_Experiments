...
const int meshWidth = 128;
const int meshHeight = 128;
...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"color_out = texture(u_textureSampler, vec3(a_texcoord.x, a_texcoord.y - u_offset, 0.0f));" \
		"gl_Position = u_mvpMatrix * vec4(a_position.x, a_position.y + (color_out.r + color_out.g + color_out.b + color_out.a) * 0.15f , a_position.z, 1.0f);" \
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(color_out.r * 0.5, color_out.g * 0.0, color_out.b * 2.0, color_out.a);" \
		"}";
...
void display(void)
{
	...
	translationMatrix = translate(-0.5f, -0.2f, -1.0f);
