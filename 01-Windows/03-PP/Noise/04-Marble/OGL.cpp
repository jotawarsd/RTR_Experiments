...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"vec4 noiseVec = texture(u_textureSampler, vec3(mc_Position));" \
		"float intensity = abs(noiseVec[0] - 0.25) + abs(noiseVec[1] - 0.125) + abs(noiseVec[2] - 0.0625) + abs(noiseVec[3] - 0.03125);" \
		"float sineval = sin(mc_Position.y * 6.0 + intensity * 12.0) * 0.5 + 0.6;" \
		"FragColor = vec4(mix(VeinColor, MarbleColor, sineval) * 0.8, 1.0);" \
		"}";
