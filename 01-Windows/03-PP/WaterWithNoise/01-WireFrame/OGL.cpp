...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"vec3 Water(float posx, float posz, float noise, float time)" \
		"{" \
			"vec3 Di = vec3(1.0, -1.0, 0.0);" \
			"vec3 pos = vec3(posx, 0.0, posz);" \
			"float Qi = 0.0, Ai = 0.0;" \
			"vec3 x = vec3(posx, 0.0, 0.0);" \
			"vec3 z = vec3(0.0, 0.0, posz);" \
			"float wi = 0.0;" \
			"float speed = 1.5;" \
			...
			"{" \
				"lamda = noise * 150.0;" \
				"Ai = (lamda / 3000) * cos(8.0 * posz - 50.0 * time);" \
				"wi = 200.0 / lamda;" \
				"phase = noise;" \
		...
		"void main(void)" \
		"{" \
		"color_out = texture(u_textureSampler, vec3(a_texcoord.x - u_offset, a_texcoord.y - u_offset, -u_offset * 0.2));" \
		"float noiseAmplitude = (color_out.r + color_out.g + color_out.b + color_out.a) * 0.1f;" \
		"vec3 waterPos = Water(a_position.x, a_position.z, noiseAmplitude, u_offset * 0.5);" \
		"gl_Position = u_mvpMatrix * vec4(a_position.x, waterPos.y, a_position.z, 1.0f);" \
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(color_out.r * 0.2f, color_out.g * 0.3f, color_out.b * 2.5f, 1.0f);" \
		"}";
	...
	for (unsigned int i = 0; i < meshWidth; i++)
	{
		for (unsigned int j = 0; j < meshHeight; j++)
		{
			...
			position[i][j][0] = vec3(k - dw, 0.0f, l - dh);
			position[i][j][1] = vec3(k - dw, 0.0f, l + dh);
			position[i][j][2] = vec3(k + dw, 0.0f, l + dh);

			position[i][j][3] = vec3(k - dw, 0.0f, l - dh);
			position[i][j][4] = vec3(k + dw, 0.0f, l - dh);
			position[i][j][5] = vec3(k + dw, 0.0f, l + dh);
...
void display(void)
{
	...
	translationMatrix = translate(-0.5f, -0.06f, -1.0f);
