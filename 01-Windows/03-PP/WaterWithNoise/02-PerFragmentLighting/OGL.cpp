...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"{" \
			...
			"gl_Position = u_projectionMatrix * u_viewMatrix * u_modelMatrix * vec4(water.waterPosition, 1.0f);" \
	...
	for (unsigned int i = 0; i < meshWidth; i++)
	{
		for (unsigned int j = 0; j < meshHeight; j++)
		{
			...
			position[i][j][0] = vec3(u - dw, 0.0f, v - dh);
			position[i][j][1] = vec3(u - dw, 0.0f, v + dh);
			position[i][j][2] = vec3(u + dw, 0.0f, v + dh);

			position[i][j][3] = vec3(u - dw, 0.0f, v - dh);
			position[i][j][4] = vec3(u + dw, 0.0f, v - dh);
			position[i][j][5] = vec3(u + dw, 0.0f, v + dh);
			...
			normals[i][j][0] = vec3(u - dw, 0.0f, v - dh);
			normals[i][j][1] = vec3(u - dw, 0.0f, v + dh);
			normals[i][j][2] = vec3(u + dw, 0.0f, v + dh);

			normals[i][j][3] = vec3(u - dw, 0.0f, v - dh);
			normals[i][j][4] = vec3(u + dw, 0.0f, v - dh);
			normals[i][j][5] = vec3(u + dw, 0.0f, v + dh);
		}
	}
...
void display(void)
{
	...
	translationMatrix = translate(-0.5f, -0.04f, -0.7f);	
