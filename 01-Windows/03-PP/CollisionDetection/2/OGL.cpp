...
//initialize circle related global variables
...
GLuint vaoCircles[numCircles];
GLuint vboPosition[numCircles];
...
int initialize(void)
{
	...
	void uninitialize(void);
	void resize(int, int);
	void createCircle(Circle, GLuint *, GLuint *);
	...
	*/
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(o_texcoord, 1.0f, 1.0f);" \
		"}";
	...
	//create circles
	srand(time(NULL));
	circles[0].position = vec2(0.7f, 0.4f);
	circles[1].position = vec2(-0.2f, -0.7f);
	for (int i = 0; i < numCircles; i++)
	{
		// float rX = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
		// float rY = ((float)rand() / RAND_MAX) * 2.0f - 1.0f;
		circles[i].radius = 0.2f;
		// circles[i].position = vec2(rX, rY);
		circles[i].front = vec2(0.0f, 0.0f);
		circles[i].mass = 2.0f;
		createCircle(circles[i], &vaoCircles[i], &vboPosition[i]);
	}
...
void createCircle(Circle circle, GLuint *vao, GLuint *vbo)
{
	//code
	float x = circle.position[0];
	float y = circle.position[1];
	...
	float position[numSegments * 3];

	int vertexptr = 0;

	for (int i = 0; i < numSegments; i++)
	{
		position[vertexptr] = x + (radius * cos(angle));
		position[vertexptr + 1] = y + (radius * sin(angle));
		position[vertexptr + 2] = 0.0f;

		vertexptr += 3;
		angle += angleIncrement;
	}

	int size = sizeof(float) * 3 * numSegments;
	...
	g_vertexCount = numSegments;
}

BOOL cvc(Circle a, Circle b)
{
	//code
	...
	return r <= ((ax - bx) * (ax - bx) + (ay - by) * (ay - by));
}

void resolveCollision(Circle *A, Circle *B)
{
	// Calculate relative velocity 
	vec2 rv = (B->front - A->front) * velocity;
	...
	A->front -= 1 / A->mass * impulse;
	B->front += 1 / B->mass * impulse;
}
...
void display(void)
{
	...
	//bind vao
	if (cvc(circles[0], circles[1]))
	{
		resolveCollision(&circles[0], &circles[1]);
	}
	for (int i = 0; i < numCircles; i++)
	{
		translationMatrix = translate(vec3((circles[i].front - circles[i].position) * velocity, -3.0f));
		circles[i].position += circles[i].front * velocity;
...
void uninitialize(void)
{
	...
	for (int i = 0; i < numCircles; i++)
	{
		if (vboPosition[i])
		{
			glDeleteBuffers(1, &vboPosition[i]);
			vboPosition[i] = 0;
		}
		...
		if (vaoCircles[i])
		{
			glDeleteVertexArrays(1, &vaoCircles[i]);
			vaoCircles[i] = 0;
		}
	}
