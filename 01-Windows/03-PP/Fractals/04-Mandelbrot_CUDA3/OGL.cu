...
#define MYARRAYSIZE (meshWidth * meshHeight * 3)
...
//CUDA Kernel
__global__ void sineWaveKernel(float3 *color, unsigned int width, unsigned int height, int maxIter)
{
	//code
	...
	float r = iter * 0.3 * 255;
	float g = iter * 0.5 * 255;
	float b = iter * 255;

	color[(j * width) + i] = make_float3(r, g, b);
}
...
int initialize(void)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu);
		glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE * sizeof(float), NULL, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void display(void)
{
	...
	sineWaveKernel <<<grid, block>>> (pPos, meshWidth, meshHeight, numIterations);
