...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"color.b += minDist;" \
		"vec3 outColor = mix(vec3(0.0, 0.35, 0.5), vec3(1.0, 1.0, 1.0), color.b);" \
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_texture);
		glBufferData(GL_ARRAY_BUFFER, texCoords->size * sizeof(float), (GLfloat *)texCoords->pf, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vbo_indices_texcoord);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_indices_texcoord);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, textureIndices->size * sizeof(float), (GLfloat *)textureIndices->p, GL_STATIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
...
void display(void)
{
	...
	//Draw
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_indices_position);
	glDrawElements(GL_TRIANGLE_STRIP, vertexIndices->size, GL_UNSIGNED_INT, 0);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
