...
GLuint g_cyLinderStacks = 30;
GLuint g_cylinderSlices = 30;
...
GLuint eFactorUniform;
GLuint eOffsetUniform;
...
int initialize(void)
{
	...
	void uninitialize(void);
	void resize(int, int);
	void createCylinder(GLuint, GLuint, GLfloat, GLfloat);
	...
	const GLchar* vertexShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"vec4 position = a_position + u_factor * sin(u_offset) * vec4(normalize(a_position.xyz), 0.0f);" \
	...
	*/
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(o_texcoord, 1.0f, 1.0f) * o_normal.x;" \
		"}";
	...
	eFactorUniform = glGetUniformLocation(shaderProgramObject, "u_factor");
	eOffsetUniform = glGetUniformLocation(shaderProgramObject, "u_offset");

	createCylinder(g_cyLinderStacks, g_cylinderSlices, g_radius, g_height);
...
void createCylinder(GLuint stacks, GLuint slices, GLfloat radius, GLfloat height)
{
	//variable declarations
	float x, y, z;
	float s, t;
	...
	for (i = 0; i < stacks - 1; i++)
	{
		for (j = 0; j < slices - 1; j++)
		{
			...
			position[3 * vertexptr + 1] = y * height;
...
void display(void)
{
	...
	translationMatrix = translate(0.0f, 1.0f, -10.0f);
	modelViewMatrix = translationMatrix;
	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
	glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
	glUniform1f(eFactorUniform, e_factor);
	glUniform1f(eOffsetUniform, e_offset);
...
{
	//code
	static float incrementFactor = 1 / (2 * M_PI);
	e_offset += 0.1f * incrementFactor;
	if (e_offset > 2 * M_PI)
	{
		e_offset -= 2 * M_PI;
	}
