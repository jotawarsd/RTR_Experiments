...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"color_out = texture(u_textureSampler, vec3(a_texcoord.x, a_texcoord.y, 1.0f));" \
		"gl_Position = u_mvpMatrix * vec4(a_position.x, a_position.y + (color_out.r + color_out.g + color_out.b + color_out.a) * 0.1f , a_position.z, 1.0f);" \
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"float color = color_out.r + color_out.g + color_out.b + color_out.a;" \
		"FragColor = vec4(0.85 * color, 0.543 * color, 0.1 * color, 1.0);" \
		"}";
...
void display(void)
{
	...
	translationMatrix = translate(-0.5f, -0.15f, -1.0f);
