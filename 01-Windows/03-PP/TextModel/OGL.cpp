...
GLfloat lightPosition[] = { 0.0f, 5.0f, 20.0f, 1.0f };
...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"}" \
		"o_worldPosition = u_modelMatrix * a_position;" \
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"FragColor = texture(u_noiseTexture, o_worldPosition.xyz);" \
		"if (FragColor.r < sval)" \
		"{discard;}" \
		"FragColor.rgb *= phong_ads_color * 2.0f;" \
		"}";
	...
	noiseTextureUniform = glGetUniformLocation(shaderProgramObject, "u_noiseTexture");
	svalUniform = glGetUniformLocation(shaderProgramObject, "sval");
	...
	unsigned char *noiseData = NULL;

	make3DNoise(NOISE_SIZE, &noiseData);

	if (noiseData == NULL)
	{
		fprintf(gpFile, "\nNoise Creation Failed!! exiting now.\n");
		uninitialize();
		DestroyWindow(ghwnd);
	}
	...
	glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA, NOISE_SIZE, NOISE_SIZE, NOISE_SIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, noiseData);
...
void loadMesh(char *modelFileName)
{
	//function declarations
	void uninitialize(void);
	...
	//code
	gpMeshFile = fopen(modelFileName, "r");
	if (gpMeshFile == NULL)
	{
...
void display(void)
{
	...
	glUniform1f(svalUniform, 0.7f * sinf(val));

	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_3D, noise3DTexture);
	glUniform1i(noiseTextureUniform, 0);
