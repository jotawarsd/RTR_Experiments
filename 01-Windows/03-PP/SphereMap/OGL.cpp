...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"void main(void)" \
		"{" \
		"FragColor = texture(u_textureSampler, o_texcoord);" \
		"}";
	...
	//post link steps--u_projectionMatrix * u_viewMatrix * u_modelMatrix
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vbo_sphere_texcoord), sphere_textures, GL_STATIC_DRAW);
	...
	//load texture
	if (LoadGLTexture(&textureSphere, MAKEINTRESOURCE(IDBITMAP_SPHERE)) == FALSE)
		return -6;
...
void display(void)
{
	...
	//transformations
	mat4 translationMatrix = translate(0.0f, 0.0f, 0.0f);
