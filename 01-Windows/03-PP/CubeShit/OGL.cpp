...
int initialize(void)
{
	...
	*/
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"vec2 t = a_texcoord_out * 2.0f - 1.0f;" \
		"t = t * 10.0f;" \
		"float brightness = clamp(1.0f - length(fract(t) - 0.5), 0.5f, 1.0f) - 0.5f;" \
		"vec2 uv = vec2(fract(a_texcoord_out.x + u_offset), a_texcoord_out.y);" \
		"FragColor = vec4(0.0f, brightness.x * 2.0f * uv.y, brightness.x * 2.0f * uv.x, 1.0f);" \
		"}";
