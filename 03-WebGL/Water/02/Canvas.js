...
function initialize()
{
	...
	var vertexShaderSourceCode =
		...
		"Ai = (lamda / 1500.0) * (2.0 * cos(8.0 * pos_z + 50.0 * u_time));" +
		"wi = (128.0 / lamda);" +
		"phase = wi * speed;" +
		"Qi = 0.1;" +
		...
		"py += piy;" +
		"pz += piz;" +
		"nix = dot(Di, x) * wi * Ai * sin((wi * dot(Di, pos)) + (phase * u_time));" +
		"niy = Qi * wi * Ai * sin((wi * dot(Di, pos)) + (phase * u_time));" +
		"niz = dot(Di, z) * wi * Ai * sin((wi * dot(Di, pos)) + (phase * u_time));" +
		"nx += nix;" +
		"ny += niy;" +
		"nz += niz;" +
		"}" +
		"gl_PointSize = 2.0;" +
		"vec4 position = vec4(pos_x + px, py, pos_z + pz, 1.0);" +
		"gl_Position = u_projectionMatrix * u_viewMatrix * u_modelMatrix * position;" +
		"vec3 o_normal = vec3(-nx, 1.0 - ny, -nz);" +
	...
	for (let z = 1; z < meshHeight - 1; z++)
	{
		for (let x = 1; x < meshWidth - 1; x++)
		{
			...
			normals.push(u - 0.5 / meshWidth, 0.0, v - 0.5 / meshHeight);
			normals.push(u + 0.5 / meshWidth, 0.0, v + 0.5 / meshHeight);
			normals.push(u + 0.5 / meshWidth, 0.0, v - 0.5 / meshHeight);
