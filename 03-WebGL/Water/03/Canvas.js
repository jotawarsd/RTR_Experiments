...
function initialize()
{
	...
	var vertexShaderSourceCode =
		...
		"}" +
		"gl_PointSize = 2.0;" +
		//"vec4 position = vec4(float(int(pos_x * float(u_meshSize)) / 4) / float(u_meshSize / 4), py * 0.006, float(int(pos_z * float(u_meshSize)) / 4) / float(u_meshSize / 4), 1.0);" +
		"vec4 position = vec4(pos_x, py * 0.005, pos_z, 1.0);" +
